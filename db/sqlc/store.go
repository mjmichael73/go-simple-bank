...
func (store *Store) execTx(ctx context.Context, fn func(*Queries) error) error {
	...
	if err != nil {
		if rbErr := tx.Rollback(); rbErr != nil {
			return fmt.Errorf("tx error: %w, rb error: %v", err, rbErr)
		}
		return err
	}
	return tx.Commit()
...
// It contains a transfer record, add account entries, and update account's balance within a single database transaction
